...
import static org.memgraph.jdbc.bolt.utils.Mocker.*;

/**
 * @author AgileLARUS
 * @since 3.0.0
 */
public class BoltProtocolGraphConnectionTest {

	@Rule public ExpectedException expectedEx = ExpectedException.none();

	private BoltProtocolGraphConnectionImpl openConnection;
	private BoltProtocolGraphConnectionImpl closedConnection;
	private BoltProtocolGraphConnectionImpl slowOpenConnection;
	private BoltProtocolGraphConnectionImpl exceptionOpenConnection;

	@Before public void setUp() {
		openConnection = new BoltProtocolGraphConnectionImpl(mockDriverOpen(), new Properties(), "");
		closedConnection = new BoltProtocolGraphConnectionImpl(mockDriverClosed(), new Properties(), "");
		slowOpenConnection = new BoltProtocolGraphConnectionImpl(mockDriverOpenSlow(), new Properties(), "");
		exceptionOpenConnection = new BoltProtocolGraphConnectionImpl(mockDriverException(), new Properties(), "");
	}
	...
	@Test public void closeShouldCloseConnection() throws SQLException {
		...
		Connection connection = new BoltProtocolGraphConnectionImpl(driver, new Properties(), "");
		connection.close();
	...
	@Test public void closeShouldThrowExceptionWhenDatabaseAccessErrorOccurred() throws SQLException {
		...
		Connection boltConnection = Mockito.mock(BoltProtocolGraphConnectionImpl.class);
		...
		Connection connectionFlatten = new BoltProtocolGraphConnectionImpl(session);
    ...
    */
	...
	@Test public void createStatementWithWrongSyntax() throws SQLException {
		try (Connection connection = new BoltProtocolGraphConnectionImpl(mockDriverOpen(), new Properties(), "")) {
			try {
