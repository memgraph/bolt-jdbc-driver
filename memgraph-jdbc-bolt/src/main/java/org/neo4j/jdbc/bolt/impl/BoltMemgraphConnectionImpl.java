...
import org.neo4j.jdbc.MemgraphDatabaseMetaData;
import org.neo4j.jdbc.MemgraphResultSet;
import org.neo4j.jdbc.bolt.BoltMemgraphConnection;
import org.neo4j.jdbc.bolt.BoltMemgraphDatabaseMetaData;
import org.neo4j.jdbc.bolt.BoltMemgraphPreparedStatement;
import org.neo4j.jdbc.bolt.BoltMemgraphResultSet;
import org.neo4j.jdbc.bolt.BoltMemgraphStatement;
import org.neo4j.jdbc.utils.BoltMemgraphUtils;
import org.neo4j.jdbc.boltrouting.BoltRoutingMemgraphDriver;
import org.neo4j.jdbc.impl.MemgraphConnectionImpl;
 ...
 * @since 3.0.0
 */
public class BoltMemgraphConnectionImpl extends MemgraphConnectionImpl implements BoltMemgraphConnection {
	...
	private BoltMemgraphDatabaseMetaData metadata;
	private boolean readOnly = false;
	private boolean useBookmarks = true;
	private boolean closed = false;

	private static final Logger LOGGER = Logger.getLogger(BoltMemgraphConnectionImpl.class.getName());
	 ...
	 */
	public BoltMemgraphConnectionImpl(Driver driver, Properties properties, String url) {
		super(properties, url, BoltMemgraphResultSet.DEFAULT_HOLDABILITY);
	...
	public static BoltMemgraphConnection newInstance(Driver driver, Properties info, String url) {
		BoltMemgraphConnection boltConnection = new BoltMemgraphConnectionImpl(driver, info, url);
		return (BoltMemgraphConnection) Proxy
				.newProxyInstance(BoltMemgraphConnectionImpl.class.getClassLoader(), new Class[] { Connection.class, BoltMemgraphConnection.class },
						new MemgraphInvocationHandler(boltConnection, BoltMemgraphConnectionImpl.hasDebug(info)));
	}
	 ...
	 */
	@Override
	public Session newMemgraphSession() {
		try {
	...
	private void setDatabase(SessionConfig.Builder config) {
		String database = (String) this.getProperties().get(BoltMemgraphDriverImpl.DATABASE);
	...
	private Bookmark[] extractBookmarks() throws SQLException {
		String stringBookmark = this.getClientInfo(BoltRoutingMemgraphDriver.BOOKMARK);
	...
	@Override public MemgraphDatabaseMetaData getMetaData() throws SQLException {
		if(metadata == null){
			metadata = new BoltMemgraphDatabaseMetaData(this);
		}
		return metadata;
	}
	...
	private void setBookmark() throws SQLClientInfoException {
		...
		if (internalBookmark != null && internalBookmark.values().iterator().hasNext()) {
			...
			this.setClientInfo(BoltRoutingMemgraphDriver.BOOKMARK, bookmark);
		}
	}
	...
	@Override public Statement createStatement() throws SQLException {
		return createStatement(MemgraphResultSet.TYPE_FORWARD_ONLY, MemgraphResultSet.CONCUR_READ_ONLY, MemgraphResultSet.CLOSE_CURSORS_AT_COMMIT);
	}

	@Override public Statement createStatement(int resultSetType, int resultSetConcurrency) throws SQLException {
		return createStatement(resultSetType, resultSetConcurrency, MemgraphResultSet.CLOSE_CURSORS_AT_COMMIT);
	}
	...
	@Override public Statement createStatement(int resultSetType, int resultSetConcurrency, int resultSetHoldability) throws SQLException {
		...
		return BoltMemgraphStatement.newInstance(false, this, resultSetType, resultSetConcurrency, resultSetHoldability);
	}
	...
	@Override public PreparedStatement prepareStatement(String sql) throws SQLException {
		return prepareStatement(nativeSQL(sql), MemgraphResultSet.TYPE_FORWARD_ONLY, MemgraphResultSet.CONCUR_READ_ONLY, MemgraphResultSet.CLOSE_CURSORS_AT_COMMIT);
	}
	...
	@Override public PreparedStatement prepareStatement(String sql, int resultSetType, int resultSetConcurrency) throws SQLException {
		return prepareStatement(nativeSQL(sql), resultSetType, resultSetConcurrency, MemgraphResultSet.CLOSE_CURSORS_AT_COMMIT);
	}
	...
	@Override public PreparedStatement prepareStatement(String sql, int resultSetType, int resultSetConcurrency, int resultSetHoldability) throws SQLException {
		...
		this.checkHoldabilityParams(resultSetHoldability);
		this.initTransaction();
		return BoltMemgraphPreparedStatement.newInstance(false, this, nativeSQL(sql), resultSetType, resultSetConcurrency, resultSetHoldability);
	}
	...
	private void closeSession() {
		if (this.session != null && this.session.isOpen()) {
			BoltMemgraphUtils.closeSafely(this.session, LOGGER);
		}
		this.session = null;
	}
	...
	private void closeTransaction() {
		if (this.transaction != null && this.transaction.isOpen()) {
			BoltMemgraphUtils.closeSafely(this.transaction, LOGGER);
		}
	...
	@Override public boolean isValid(int timeout) throws SQLException {
		...
		Runnable r = () -> {
			try (Session s = newMemgraphSession(); Transaction tr = s.beginTransaction()) {
	...
	private void initSession() {
		this.session = newMemgraphSession();
    }
