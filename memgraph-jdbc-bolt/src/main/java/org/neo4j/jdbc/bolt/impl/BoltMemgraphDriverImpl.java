...
import org.neo4j.jdbc.MemgraphDriver;
 ...
 * @since 3.3.1
 */
public abstract class BoltMemgraphDriverImpl extends MemgraphDriver {
    ...
    protected BoltMemgraphDriverImpl(String prefix) {
        super(prefix);
    }
    ...
    public Connection connect(String url, Properties props) throws SQLException {
        ...
        if (acceptsURL(url)) {
            String boltUrl = url.replace(MemgraphDriver.JDBC_PREFIX, "").replaceAll("^(" + getPrefix() + ":)([^/])", "$1//$2");
            Driver driver = null;
            try {
                ...
                connection = BoltMemgraphConnectionImpl.newInstance(driver, info, url);
            } catch (Exception e) {
                if (driver != null) {
                    try {
                        driver.close();
