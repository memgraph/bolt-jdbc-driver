 ...
 */
public class BoltProtocolGraphUtils {
    ...
    public static int calculateUpdateCount(SummaryCounters stats) {
		...
		/*
		Memgraph has a detailed report to explain which entity is changed.
    ...
    public static <R> R executeInTx(BoltProtocolGraphConnection connection,
                                    String sql,
    ...
    private static Result execute(BoltProtocolGraphConnection connection,
                                          String statement,
    ...
    public static boolean hasResultSet(BoltProtocolGraphConnection connection, String cypher) {
        if (StringUtils.isBlank(cypher)) {
            return false;
        }
        try (final Session session = connection.newMemgraphSession();
