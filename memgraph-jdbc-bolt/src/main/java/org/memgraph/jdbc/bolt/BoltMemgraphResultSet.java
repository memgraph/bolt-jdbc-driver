...
import org.memgraph.jdbc.MemgraphArray;
import org.memgraph.jdbc.MemgraphConnection;
import org.memgraph.jdbc.MemgraphResultSet;
import org.memgraph.jdbc.impl.ListArray;
import org.memgraph.jdbc.utils.JSONUtils;
import org.memgraph.jdbc.utils.MemgraphInvocationHandler;
import org.memgraph.jdbc.utils.ObjectConverter;
...
import static org.memgraph.jdbc.utils.DataConverterUtils.convertObject;
import static org.memgraph.jdbc.utils.DataConverterUtils.valueToDate;
import static org.memgraph.jdbc.utils.DataConverterUtils.valueToTime;
import static org.memgraph.jdbc.utils.DataConverterUtils.valueToTimestamp;
/**
 * @author AgileLARUS
 * @since 3.0.0
 */
public class BoltMemgraphResultSet extends MemgraphResultSet {
	 ...
	 */
	// visible for testing
	BoltMemgraphResultSet(Statement statement, Result iterator, int... params) {
		...
		try {
			this.flatten = ((MemgraphConnection) this.statement.getConnection()).getFlattening();
		} catch (Exception e) {
			this.flatten = 0;
		}
		...
		this.metaData = BoltMemgraphResultSetMetaData.newInstance(false, this.classes, this.keys);
	}

	public static ResultSet newInstance(boolean debug, Statement statement, Result iterator, int... params) {
		ResultSet rs = new BoltMemgraphResultSet(statement, iterator, params);
		return (ResultSet) Proxy
				.newProxyInstance(BoltMemgraphResultSet.class.getClassLoader(), new Class[] { ResultSet.class }, new MemgraphInvocationHandler(rs, debug));
	}
	...
	@Override public MemgraphArray getArray(int columnIndex) throws SQLException {
		checkClosed();
		List<Object> list = this.fetchValueFromIndex(columnIndex).asList();
		Object obj = (list.isEmpty())?new Object():list.get(0);

		return new ListArray(list, MemgraphArray.getObjectType(obj));
	}

	@SuppressWarnings("rawtypes") @Override public MemgraphArray getArray(String columnLabel) throws SQLException {
		checkClosed();
	 ...
	 * @return
	 */
	private boolean isMemgraphDatatype(Class type){
