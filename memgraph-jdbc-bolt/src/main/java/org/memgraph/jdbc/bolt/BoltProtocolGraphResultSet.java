...
import org.memgraph.jdbc.GraphConnection;
 ...
 * @since 3.0.0
 */
public class BoltProtocolGraphResultSet extends GraphResultSet {
	 ...
	 */
	// visible for testing
	BoltProtocolGraphResultSet(Statement statement, Result iterator, int... params) {
		...
		try {
			this.flatten = ((GraphConnection) this.statement.getConnection()).getFlattening();
		} catch (Exception e) {
			this.flatten = 0;
		}
		...
		this.metaData = BoltProtocolGraphResultSetMetaData.newInstance(false, this.classes, this.keys);
	}

	public static ResultSet newInstance(boolean debug, Statement statement, Result iterator, int... params) {
		ResultSet rs = new BoltProtocolGraphResultSet(statement, iterator, params);
		return (ResultSet) Proxy
				.newProxyInstance(BoltProtocolGraphResultSet.class.getClassLoader(), new Class[] { ResultSet.class }, new GraphInvocationHandler(rs, debug));
	}
	...
	@Override public GraphArray getArray(int columnIndex) throws SQLException {
		checkClosed();
		List<Object> list = this.fetchValueFromIndex(columnIndex).asList();
		Object obj = (list.isEmpty())?new Object():list.get(0);

		return new ListArray(list, GraphArray.getObjectType(obj));
	}

	@SuppressWarnings("rawtypes") @Override public GraphArray getArray(String columnLabel) throws SQLException {
		checkClosed();
