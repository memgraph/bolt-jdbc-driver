...
import static org.memgraph.jdbc.utils.BoltProtocolGraphUtils.executeInTx;
import static org.memgraph.jdbc.utils.BoltProtocolGraphUtils.hasResultSet;

public class BoltProtocolGraphPreparedStatement extends GraphPreparedStatement implements Loggable {

	private final ResultSetFactory resultSetFactory;

	private BoltProtocolGraphPreparedStatement(BoltProtocolGraphConnectionImpl connection, String rawStatement, int... rsParams) {
		this(connection, BoltProtocolGraphResultSet::newInstance, rawStatement, rsParams);
	}

	// visible for testing
	BoltProtocolGraphPreparedStatement(BoltProtocolGraphConnectionImpl connection, ResultSetFactory resultSetFactory, String rawStatement, int... rsParams) {
		super(connection, rawStatement);
		this.resultSetFactory = resultSetFactory;
		this.resultSetParams = rsParams;
	}

	public static PreparedStatement newInstance(boolean debug, BoltProtocolGraphConnectionImpl connection, String rawStatement, int... rsParams) {
		PreparedStatement ps = new BoltProtocolGraphPreparedStatement(connection, rawStatement, rsParams);
		((GraphPreparedStatement) ps).setDebug(debug);
		return (PreparedStatement) Proxy.newProxyInstance(BoltProtocolGraphPreparedStatement.class.getClassLoader(), new Class[] { PreparedStatement.class },
	...
	@Override public int executeUpdate() throws SQLException {
		return executeInternal((result) -> {
			...
			this.currentUpdateCount = BoltProtocolGraphUtils.calculateUpdateCount(stats);
	...
	@Override public boolean execute() throws SQLException {
		return executeInternal((result) -> {
			boolean hasResultSet = hasResultSet((BoltProtocolGraphConnection) this.connection, this.statement);
			if (hasResultSet) {
				this.currentResultSet = this.resultSetFactory.create(this.hasDebug(), this, result, this.resultSetParams);
				this.currentUpdateCount = -1;
			} else {
	...
	private <T> T executeInternal(Function<Result, T> body) throws SQLException {
		this.checkClosed();
		return executeInTx((BoltProtocolGraphConnection) this.connection, this.statement, this.parameters, body);
	}

	@Override public GraphParameterMetaData getParameterMetaData() throws SQLException {
		this.checkClosed();
		return new BoltProtocolGraphParameterMetaData(this);
	}
	...
	@Override public int[] executeBatch() throws SQLException {
		...
		try {
			BoltProtocolGraphConnection connection = (BoltProtocolGraphConnection) this.connection;
