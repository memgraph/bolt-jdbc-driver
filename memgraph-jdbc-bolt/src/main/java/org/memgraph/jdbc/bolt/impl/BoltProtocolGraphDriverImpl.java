...
import org.memgraph.jdbc.GraphDriver;
 ...
 * @since 3.3.1
 */
public abstract class BoltProtocolGraphDriverImpl extends GraphDriver {
    ...
    protected BoltProtocolGraphDriverImpl(String prefix) {
        super(prefix);
    }
    ...
    public Connection connect(String url, Properties props) throws SQLException {
        ...
        if (acceptsURL(url)) {
            String boltUrl = url.replace(GraphDriver.JDBC_PREFIX, "").replaceAll("^(" + getPrefix() + ":)([^/])", "$1//$2");
            Driver driver = null;
            try {
                ...
                connection = BoltProtocolGraphConnectionImpl.newInstance(driver, info, url);
            } catch (Exception e) {
                if (driver != null) {
                    try {
                        driver.close();
