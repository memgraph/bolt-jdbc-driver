...
import org.memgraph.jdbc.bolt.BoltProtocolGraphDatabaseMetaData;
import org.memgraph.jdbc.bolt.BoltProtocolGraphPreparedStatement;
import org.memgraph.jdbc.bolt.BoltProtocolGraphResultSet;
import org.memgraph.jdbc.bolt.BoltProtocolGraphStatement;
 ...
 * @since 3.0.0
 */
public class BoltProtocolGraphConnectionImpl extends GraphConnectionImpl implements BoltProtocolGraphConnection {
	...
	private BoltProtocolGraphDatabaseMetaData metadata;
	private boolean readOnly = false;
	private boolean useBookmarks = true;
	private boolean closed = false;

	private static final Logger LOGGER = Logger.getLogger(BoltProtocolGraphConnectionImpl.class.getName());
	 ...
	 */
	public BoltProtocolGraphConnectionImpl(Driver driver, Properties properties, String url) {
		super(properties, url, BoltProtocolGraphResultSet.DEFAULT_HOLDABILITY);
	...
	public static BoltProtocolGraphConnection newInstance(Driver driver, Properties info, String url) {
		BoltProtocolGraphConnection boltConnection = new BoltProtocolGraphConnectionImpl(driver, info, url);
		return (BoltProtocolGraphConnection) Proxy
				.newProxyInstance(BoltProtocolGraphConnectionImpl.class.getClassLoader(), new Class[] { Connection.class, BoltProtocolGraphConnection.class },
						new GraphInvocationHandler(boltConnection, BoltProtocolGraphConnectionImpl.hasDebug(info)));
	}
	...
	private void setDatabase(SessionConfig.Builder config) {
		String database = (String) this.getProperties().get(BoltProtocolGraphDriverImpl.DATABASE);
	...
	@Override public GraphDatabaseMetaData getMetaData() throws SQLException {
		if(metadata == null){
			metadata = new BoltProtocolGraphDatabaseMetaData(this);
		}
		return metadata;
	}
	...
	@Override public Statement createStatement(int resultSetType, int resultSetConcurrency, int resultSetHoldability) throws SQLException {
		...
		return BoltProtocolGraphStatement.newInstance(false, this, resultSetType, resultSetConcurrency, resultSetHoldability);
	}
	...
	@Override public PreparedStatement prepareStatement(String sql, int resultSetType, int resultSetConcurrency, int resultSetHoldability) throws SQLException {
		...
		this.checkHoldabilityParams(resultSetHoldability);
		this.initTransaction();
		return BoltProtocolGraphPreparedStatement.newInstance(false, this, nativeSQL(sql), resultSetType, resultSetConcurrency, resultSetHoldability);
	}
	...
	private void closeSession() {
		if (this.session != null && this.session.isOpen()) {
			BoltProtocolGraphUtils.closeSafely(this.session, LOGGER);
		}
		this.session = null;
	}
	...
	private void closeTransaction() {
		if (this.transaction != null && this.transaction.isOpen()) {
			BoltProtocolGraphUtils.closeSafely(this.transaction, LOGGER);
		}
