 ...
 * @since 3.2.0
 */
public abstract class GraphConnectionImpl implements GraphConnection {
	/**
	 ...
	 */
	protected GraphConnectionImpl(Properties properties, String url, int defaultHoldability) {
		this.url = url;
	...
	protected void checkHoldabilityParams(int resultSetHoldability) throws SQLException {
		// @formatter:off
		if( resultSetHoldability != GraphResultSet.HOLD_CURSORS_OVER_COMMIT &&
			resultSetHoldability != GraphResultSet.CLOSE_CURSORS_AT_COMMIT
		){
	...
	protected void checkConcurrencyParams(int resultSetConcurrency) throws SQLException {
		// @formatter:off
		if( resultSetConcurrency != GraphResultSet.CONCUR_UPDATABLE &&
			resultSetConcurrency != GraphResultSet.CONCUR_READ_ONLY
		){
			throw new SQLFeatureNotSupportedException();
		}
		// @formatter:on
	...
	protected void checkTypeParams(int resultSetType) throws SQLException {
		// @formatter:off
		if( resultSetType != GraphResultSet.TYPE_FORWARD_ONLY &&
			resultSetType != GraphResultSet.TYPE_SCROLL_INSENSITIVE &&
			resultSetType != GraphResultSet.TYPE_SCROLL_SENSITIVE
		){
			throw new SQLFeatureNotSupportedException();
		}
		// @formatter:on
	 ...
	 * Graph doesn't implement catalog feature, so we do nothing to avoid some tools exception.
	 */
	 ...
	 * Graph doesn't implement catalog feature, so return <code>null</code> (@see {@link java.sql.Connection#getCatalog})
	 */
	...
	@Override public GraphArray createArrayOf(String typeName, Object[] elements) throws SQLException {
