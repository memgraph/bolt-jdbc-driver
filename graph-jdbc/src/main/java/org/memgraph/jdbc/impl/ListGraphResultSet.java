 ...
 */
//FIXME tests needed
public class ListGraphResultSet extends GraphResultSet {
	...
	private ListGraphResultSet(List<List<Object>> list, List<String> keys) {
		super(null);
		this.list = list;
		this.keys = keys;
	}
	...
	public static ResultSet newInstance(boolean debug, List<List<Object>> list, List<String> keys) {
		ResultSet rs = new ListGraphResultSet(list, keys);
		return (ResultSet) Proxy
				.newProxyInstance(ListGraphResultSet.class.getClassLoader(), new Class[] { ResultSet.class }, new GraphInvocationHandler(rs, debug));
	}
	...
	@Override public GraphArray getArray(String columnLabel) throws SQLException {
		return null;
	}
	...
	@Override public GraphResultSetMetaData getMetaData() throws SQLException {
		return new ListGraphResultSetMetaData(this.keys);
	}
