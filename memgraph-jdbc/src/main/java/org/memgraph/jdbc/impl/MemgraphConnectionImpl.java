 ...
 * @since 3.2.0
 */
public abstract class MemgraphConnectionImpl implements MemgraphConnection {
	/**
	 ...
	 */
	protected MemgraphConnectionImpl(Properties properties, String url, int defaultHoldability) {
		this.url = url;
	...
	protected void checkHoldabilityParams(int resultSetHoldability) throws SQLException {
		// @formatter:off
		if( resultSetHoldability != MemgraphResultSet.HOLD_CURSORS_OVER_COMMIT &&
			resultSetHoldability != MemgraphResultSet.CLOSE_CURSORS_AT_COMMIT
		){
	...
	protected void checkConcurrencyParams(int resultSetConcurrency) throws SQLException {
		// @formatter:off
		if( resultSetConcurrency != MemgraphResultSet.CONCUR_UPDATABLE &&
			resultSetConcurrency != MemgraphResultSet.CONCUR_READ_ONLY
		){
			throw new SQLFeatureNotSupportedException();
		}
		// @formatter:on
	...
	protected void checkTypeParams(int resultSetType) throws SQLException {
		// @formatter:off
		if( resultSetType != MemgraphResultSet.TYPE_FORWARD_ONLY &&
			resultSetType != MemgraphResultSet.TYPE_SCROLL_INSENSITIVE &&
			resultSetType != MemgraphResultSet.TYPE_SCROLL_SENSITIVE
		){
			throw new SQLFeatureNotSupportedException();
		}
		// @formatter:on
	 ...
	 * Memgraph doesn't implement catalog feature, so we do nothing to avoid some tools exception.
	 */
	 ...
	 * Memgraph doesn't implement catalog feature, so return <code>null</code> (@see {@link java.sql.Connection#getCatalog})
	 */
	...
	@Override public MemgraphArray createArrayOf(String typeName, Object[] elements) throws SQLException {
