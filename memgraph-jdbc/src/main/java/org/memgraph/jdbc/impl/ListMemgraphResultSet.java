 ...
 */
//FIXME tests needed
public class ListMemgraphResultSet extends MemgraphResultSet {
	...
	private ListMemgraphResultSet(List<List<Object>> list, List<String> keys) {
		super(null);
		this.list = list;
		this.keys = keys;
	}
	...
	public static ResultSet newInstance(boolean debug, List<List<Object>> list, List<String> keys) {
		ResultSet rs = new ListMemgraphResultSet(list, keys);
		return (ResultSet) Proxy
				.newProxyInstance(ListMemgraphResultSet.class.getClassLoader(), new Class[] { ResultSet.class }, new MemgraphInvocationHandler(rs, debug));
	}
	...
	@Override public MemgraphArray getArray(String columnLabel) throws SQLException {
		return null;
	}
	...
	@Override public MemgraphResultSetMetaData getMetaData() throws SQLException {
		return new ListMemgraphResultSetMetaData(this.keys);
	}
